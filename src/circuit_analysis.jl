...
qubits(meas::QC.sMX) = [meas.qubit]
qubits(meas::QC.sMY) = [meas.qubit]
qubits(meas::QC.sMZ) = [meas.qubit]
...
function preparation_errors(circuit::Circuit)
...
function open_wire_errors(nq, qs)
	reduce(union, map(q -> paulis_on(nq, q), qs), init=[])
end
